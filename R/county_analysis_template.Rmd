...
region <- getcdw::get_cdw("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/region.sql")
imp_cap <- getcdw::get_cdw("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/imp_cap.sql")
mgs <- getcdw::get_cdw("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/mgs.sql")

events <- function(from, to) {
  
  events_query <- getcdw::parameterize_template("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/events.sql")
  getcdw::get_cdw(events_query(from = from, to = to))
}

events <- events(from = from, to = to)


visits <- function(from, to) {
  
  visits_query <- getcdw::parameterize_template("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/visits.sql")
  getcdw::get_cdw(visits_query(from = from, to = to))
}

visits <- visits(from = from, to = to)

gifts <- function(from, to) {
  
  gifts_query <- getcdw::parameterize_template("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/gifts.sql")
  getcdw::get_cdw(gifts_query(from = from, to = to))
}

gifts <- gifts(from = from, to = to)

proposals <- function(from, to) {
  
  proposals_query <- getcdw::parameterize_template("R:/Prospect Development/Prospect Analysis/regionalreports/sql/county/proposals.sql")
  getcdw::get_cdw(proposals_query(from = from, to = to))
}

proposals <- proposals(from = from, to = to)


## get the data
region_prospects <- county_prospects %>%
  inner_join(region, by = "entity_id")
...
imp_cap <- county_prospects %>%
  inner_join(imp_cap, by = "entity_id")
mgs <- county_prospects %>%
  inner_join(mgs, by = "entity_id")
events <- county_prospects %>%
  inner_join(events, by = "entity_id")
visits <- county_prospects %>%
  inner_join(visits, by = "entity_id")
gifts <- county_prospects %>%
  inner_join(gifts, by = "entity_id")
proposals <- county_prospects %>%
  inner_join(proposals, by = "entity_id")
...
This analysis looks at all alumni and current parents with a primary address in `r region_name`. The time period included is from `r format(as.Date(as.character(from), "%Y%m%d"),  "%B %d, %Y")` to `r format(as.Date(as.character(to), "%Y%m%d"),  "%B %d, %Y")`.

There are currently `r comma(n_distinct(region_prospects$entity_id))` alumni and current parents in `r regional_name`, comprising `r comma(n_distinct(region_prospects$hh_id))` households.

## Demographics

`r percent(as.numeric(record_type[1,2]/n_distinct(region_prospects$entity_id)))` of entities living in `r region_name` have a record type of `r record_type[1,1]`, `r percent(as.numeric(record_type[2,2]/n_distinct(region_prospects$entity_id)))` have a record type of `r record_type[2,1]`, and `r percent(as.numeric(record_type[3,2]/n_distinct(region_prospects$entity_id)))` have a record type of `r record_type[3,1]`. Note that these percentages do not add up to 100% since each entity may have multiple record types.
...
`r percent(as.numeric(1-filter(rated, capacity_rating == "Not Rated")[1,3]/sum(rated$households)))` of households in `r region_name` have been rated. Of those, `r percent(as.numeric(sum(filter(rated, capacity_rating_code %in% c(1:9))$households)/sum(filter(rated, capacity_rating_code %in% c(1:14))$households)))` are rated 250K+ or higher.
